...
if TYPE_CHECKING:
    from alpha_evolve.programs import (
        AlphaProgram,
    )  # Use the actual class from the framework
...
def _scale_signal_cross_sectionally(
    raw_signal_vector: np.ndarray, method: str, winsor_p: float | None = None
) -> np.ndarray:
...
def _build_feature_bundle(
    ...
    if (
        "cross_btc_momentum_t" in cross_feat_set
        or "sector_momentum_diff_t" in cross_feat_set
    ):
        reference_idx = 0
        for idx, sym in enumerate(stock_symbols):
            sym_upper = sym.upper()
...
def backtest_cross_sectional_alpha(
    ...
    | None = 252,  # Default for daily equities; if None, infer from index
    ...
    for t in range(signal_matrix.shape[0]):
        scaled_signal_t = _scale_signal_cross_sectionally(
            signal_matrix[t, :], scale_method, winsor_p
        )
        if long_short_n > 0:
    ...
    for i, sym in enumerate(stock_symbols):
        ...
        ret_fwd_matrix[:, i] = np.nan_to_num(
            ret_fwd_values, nan=0.0, posinf=0.0, neginf=0.0
        )
        ...
        close_t_matrix[:, i] = np.nan_to_num(
            close_t_vals, nan=0.0, posinf=0.0, neginf=0.0
        )
        # Shift high/low by -1 so index t corresponds to the next bar extremes
        try:
            next_high_vals = (
                aligned_dfs[sym]["high"].shift(-1).loc[idx_for_returns].values
            )
            next_low_vals = (
                aligned_dfs[sym]["low"].shift(-1).loc[idx_for_returns].values
            )
        except Exception:
            next_high_vals = np.zeros_like(close_t_vals)
            next_low_vals = np.zeros_like(close_t_vals)
        next_high_matrix[:, i] = np.nan_to_num(
            next_high_vals, nan=0.0, posinf=0.0, neginf=0.0
        )
        next_low_matrix[:, i] = np.nan_to_num(
            next_low_vals, nan=0.0, posinf=0.0, neginf=0.0
        )
    ...
    stress_tail_slippage_bps = float(
        stress_cfg.get("tail_slippage_bps", stress_slippage_bps)
    )
    stress_tail_shock_scale = float(
        stress_cfg.get("tail_shock_scale", stress_shock_scale_bt * 1.5)
    )
    eps = 1e-9
    ...
    for t in range(T):
        ...
        if stop_loss_pct and stop_loss_pct > 0.0 and lag == 1:
            per_bar_stop_notional = np.sum(
                np.abs(scaled_positions[t, :]) * stop_mask_matrix[t, :]
            )
    ...
    if (
        stress_tail_fee_bps > 0.0
        or stress_tail_slippage_bps > 0.0
        or stress_tail_shock_scale > 0.0
    ):
        scenario_specs["tail"] = {
            "fee_bps": stress_tail_fee_bps,
            "slippage_bps": stress_tail_slippage_bps,
            "shock_scale": stress_tail_shock_scale
            if stress_tail_shock_scale > 0.0
            else stress_shock_scale_bt,
        }
    ...
    def _compute_scenario(
        label: str,
        fee_bps_local: float,
        slip_bps_local: float,
        shock_scale_local: float,
    ) -> Dict[str, float]:
        if len(daily_portfolio_returns_net) == 0:
            return {
                "Sharpe": 0.0,
        ...
        return {
            ...
            "CostExtra": float(np.mean(extra_costs_local))
            if extra_costs_local.size
            else 0.0,
            "ShockScale": float(scale_local),
        }
    ...
    result["Stress"] = stress_results.get(
        "base",
        {
            "Sharpe": 0.0,
            "AnnReturn": 0.0,
    ...
    baseline_tc = (
        fee_rate * abs_pos_diff_sum
        if len(daily_portfolio_returns_net) > 0
        else np.zeros(0)
    )
    result["TransactionCosts"] = {
        "baseline_cost": float(np.mean(baseline_tc)) if baseline_tc.size else 0.0,
        "turnover_mean": float(np.mean(abs_pos_diff_sum))
        if len(abs_pos_diff_sum)
        else 0.0,
    }
