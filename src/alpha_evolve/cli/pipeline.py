...
from alpha_evolve.config import EvolutionConfig, BacktestConfig
from alpha_evolve.utils.data_loading import DataLoadError
from alpha_evolve.evolution import engine as ae
...
from alpha_evolve.config.layering import load_config_file, layer_dataclass_config, _flatten_sectioned_config  # type: ignore
...
def _write_hof_snapshots(run_dir: Path) -> list[Path]:
    ...
    Uses alpha_evolve.utils.diagnostics.get_all() entries produced during evolution. Each
    entry may include a 'hof' field added by alpha_evolve.evolution.engine; if present, this
...
def _write_data_alignment_meta(run_dir: Path, evo_cfg: EvolutionConfig) -> Path:
    ...
    """
    from alpha_evolve.evolution import data as evo_dh
    di = None
    try:
...
def main(argv: Sequence[str] | None = None) -> None:
    evo_cfg, bt_cfg, ns = parse_args(argv)
