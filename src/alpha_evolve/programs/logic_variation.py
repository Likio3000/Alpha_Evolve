...
if TYPE_CHECKING:
    from .program import AlphaProgram  # For type hint of self and return type
...
def mutate_program_logic(
    ...
    evo: Optional[EvolutionParams] = (
        params if isinstance(params, EvolutionParams) else None
    )
    ...
    block_limit_map = {
        "setup": max_setup_ops,
        "predict": max_predict_ops,
        "update": max_update_ops,
    }
    ...
    possible_mutations = []
    if (
        current_total_ops < max_total_ops
        and len(chosen_block_ops_list) < block_limit_map[chosen_block_name]
    ):
    ...
    if mutation_type == "add":
        ...
        change_inputs_mutation(
            new_prog, chosen_block_name, rng, feature_vars, state_vars
        )
...
def _finalize_predict_tail(
    ...
    if prog.predict_ops:
        ...
        vars_before_last = prog.get_vars_at_point(
            "predict", len(prog.predict_ops) - 1, feature_vars, state_vars
        )
        ...
        if (
            ...
            if actual_last_op_out_type == "vector":
                ...
                source_for_final_fix = pick_vector_fallback(
                    vars_for_final_fix, feature_vars, rng=rng
                )
...
def add_op_mutation(
    ...
    ops_list = (
        prog.setup
        if block_name == "setup"
        else prog.predict_ops
        if block_name == "predict"
        else prog.update_ops
    )
    insertion_idx = int(rng.integers(0, len(ops_list) + 1))
    vars_at_insertion = prog.get_vars_at_point(
        block_name, insertion_idx, feature_vars, state_vars
    )
    ...
    for op_n, op_s in OP_REGISTRY.items():
        ...
        for req_t in op_s.in_types:
            cands = select_var_candidates(
                temp_current_vars,
                req_t,
                allow_elementwise_scalar_promotion=(
                    op_s.is_elementwise and req_t == "scalar"
                ),
            )
            if not cands:
                formable = False
    ...
    weights = np.array(
        [
            op_weight(op_n, is_predict=(block_name == "predict"))
            for op_n, _, _ in candidate_ops_for_add
        ],
        dtype=float,
    )
...
def remove_op_mutation(
    ...
    is_final_pred_op_targeted = (
        block_name == "predict"
        and ops_list[idx_to_remove].out == FINAL_PREDICTION_VECTOR_NAME
        and idx_to_remove == len(ops_list) - 1
    )
...
def change_op_mutation(
    ...
    is_final_predict_op = (
        ...
        and op_idx_to_change == len(ops_list) - 1
    )
    compatible = []
    for op_n, op_s in OP_REGISTRY.items():
    ...
    if not compatible:
        return
    weights = np.array(
        [op_weight(op_n, is_predict=(block_name == "predict")) for op_n in compatible],
        dtype=float,
    )
    if np.all(weights <= 0) or np.isnan(weights).any():
...
def change_inputs_mutation(
    ...
    if not op_to_mutate.inputs:
        return
    vars_at_op = prog.get_vars_at_point(
        block_name, op_idx_to_change, feature_vars, state_vars
    )
    ...
    eligible = select_var_candidates(
        ...
        required_type,
        allow_elementwise_scalar_promotion=(
            spec.is_elementwise and required_type == "scalar"
        ),
    ...
    ops_list[op_idx_to_change] = Op(
        op_to_mutate.out, op_to_mutate.opcode, tuple(new_inputs)
    )
