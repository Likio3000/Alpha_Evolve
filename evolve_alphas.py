...
def _random_prog(cfg: EvoConfig) -> AlphaProgram:  # Signature changed
    return AlphaProgram.random_program(
        FEATURE_VARS, INITIAL_STATE_VARS, max_total_ops=cfg.max_ops
    )
        ...
        for gen in range(cfg.generations):
            ...
            for i in bar:
                ...
                score, mean_ic, processed_preds_matrix = evaluate_program(
                    prog, dh_module, hof_module, INITIAL_STATE_VARS
                )
                ...
                if not cfg.quiet and hasattr(bar, "set_postfix_str"):
                    ...
                    best_score_so_far = (
                        np.max(valid_scores) if valid_scores.size > 0 else -np.inf
                    )
            ...
            if eval_results and eval_results[0][1] > -np.inf:
                (
                    best_prog_idx_this_gen,
                    best_fit_this_gen,
                    best_ic_this_gen,
                    best_preds_matrix_this_gen,
                ) = eval_results[0]
                ...
                if best_preds_matrix_this_gen is not None:
                    update_correlation_hof(
                        best_program_instance_this_gen.fingerprint,
                        best_preds_matrix_this_gen,
                    )
            ...
            if not eval_results or eval_results[0][1] <= -float("inf"):
                print(
                    f"Gen {gen + 1:3d} | No valid programs. Restarting population and HOF."
                )
            ...
            while len(new_pop) < cfg.pop_size:
                ...
                if (
                    len(valid_indices_for_tournament) < num_to_sample
                    and len(valid_indices_for_tournament) > 0
                ):
                ...
                if random.random() < cfg.p_cross:
                    ...
                    child = (
                        parent_a.copy() if random.random() < 0.5 else parent_b.copy()
                    )
