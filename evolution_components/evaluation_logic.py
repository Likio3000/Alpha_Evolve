...
if TYPE_CHECKING:
    from alpha_framework.alpha_framework_program import (
        AlphaProgram,
    )  # Changed from alpha_program_core
    ...
    from evolution_components import (
        hall_of_fame_manager as hof_manager,
    )  # To get HOF penalty
...
def _uses_feature_vector_check(prog: AlphaProgram) -> bool:
    ...
    all_ops_map: Dict[
        str, Op
    ] = {}  # Op needs to be imported from alpha_framework.alpha_framework_op
...
def evaluate_program(
    ...
    for t_idx in range(num_evaluation_steps):
        ...
        for feat_name_template in CROSS_SECTIONAL_FEATURE_VECTOR_NAMES:
            ...
            try:
                feat_vec = np.array(
                    [
                        aligned_dfs[sym].loc[timestamp, col_name]
        ...
        try:
            ...
            if len(all_raw_predictions_timeseries) == _EVAL_CONFIG["early_abort_bars"]:
                ...
                if (
                    ...
                    mean_xs_std_partial = np.mean(
                        partial_raw_preds_matrix.std(axis=1, ddof=0)
                    )
                ...
                if (
                    mean_xs_std_partial < _EVAL_CONFIG["early_abort_xs_threshold"]
                    or mean_t_std_partial < _EVAL_CONFIG["early_abort_t_threshold"]
                ):
                    _eval_cache[fp] = (-float("inf"), 0.0, None)
                    return _eval_cache[fp]
            ...
            actual_returns_t_slice = np.array(
                [
                    aligned_dfs[sym].loc[return_timestamp_for_ic, "ret_fwd"]
                    for sym in stock_symbols
                ],
                dtype=float,
            )
            actual_returns_t = np.nan_to_num(
                actual_returns_t_slice, nan=0.0, posinf=0.0, neginf=0.0
            )  # Clean returns

            if np.all(
                np.isnan(actual_returns_t)
            ):  # If all returns are NaN (e.g. end of data for some reason)
    ...
    score = mean_daily_ic - (
        _EVAL_CONFIG["parsimony_penalty_factor"]
        * prog.size
        / _EVAL_CONFIG["max_ops_for_parsimony"]
    )
    ...
    if score > -float("inf"):  # Temporal check only if not already penalized to -inf
        if (
            ...
            if (
                full_raw_predictions_matrix.std(ddof=0)
                < _EVAL_CONFIG["temporal_flatness_guard_threshold"]
            ):
                score = -float("inf")
    ...
    if score > -float("inf") and full_processed_predictions_matrix.size > 0:
        correlation_penalty = hof_module.get_correlation_penalty_with_hof(
            full_processed_predictions_matrix.flatten()
        )
