...
_hof_programs_data: List[
    Tuple[str, float, float, AlphaProgram, np.ndarray]
] = []  # fp, fitness, ic, prog, processed_preds_matrix
...
def initialize_hof(
    max_size: int, keep_dupes: bool, corr_penalty_weight: float, corr_cutoff: float
):
    global \
        _hof_programs_data, \
        _hof_max_size, \
    ...
    print(
        f"Hall of Fame initialized: max_size={max_size}, keep_dupes={keep_dupes}, corr_penalty_w={corr_penalty_weight}, corr_cutoff={corr_cutoff}"
    )


def _safe_corr(
    a: np.ndarray, b: np.ndarray
) -> float:  # Copied from evolve_alphas, will be used for HOF penalty
    ...
    with np.errstate(
        invalid="ignore"
    ):  # Suppress "invalid value encountered in true_divide"
...
def get_correlation_penalty_with_hof(
    ...
    penalty = 0.0
    if (
        not _hof_processed_prediction_timeseries_for_corr
        or current_prog_flat_processed_ts.std(ddof=0) < 1e-9
    ):
        return penalty
    ...
    for hof_flat_processed_ts in _hof_processed_prediction_timeseries_for_corr:
        ...
        corr_with_hof = abs(
            _safe_corr(current_prog_flat_processed_ts, hof_flat_processed_ts)
        )
        if (
            not np.isnan(corr_with_hof)
            and corr_with_hof > _corr_penalty_config["cutoff"]
        ):
...
def add_program_to_hof(
    ...
    if not _keep_dupes_in_hof_config and fp in _hof_fingerprints_set:
        ...
        if existing_idx != -1 and fitness > _hof_programs_data[existing_idx][1]:
            _hof_programs_data[existing_idx] = (
                fp,
                fitness,
                mean_ic,
                program,
                processed_preds_matrix
                if processed_preds_matrix is not None
                else np.array([]),
            )
        elif (
            existing_idx == -1
        ):  # Should not happen if fp in _hof_fingerprints_set, but as a safeguard
    ...
    if processed_preds_matrix is not None and fitness > -float("inf"):
        if fp not in _hof_corr_fingerprints:
            _hof_processed_prediction_timeseries_for_corr.append(
                processed_preds_matrix.ravel()
            )
...
def print_generation_summary(
...
):  # eval_results_sorted: (idx_in_pop, score, ic, processed_preds_matrix)
    ...
    temp_hof_candidates = []
    for pop_idx, fit, ic, preds_matrix_or_none in eval_results_sorted[
        :_TOP_TO_SHOW_PRINT
    ]:
        if fit <= -float("inf"):
        ...
        temp_hof_candidates.append(
            (
                fp,
                fit,
                ic,
                prog,
                preds_matrix_or_none
                if preds_matrix_or_none is not None
                else np.array([]),
            )
        )
    ...
    for rk, (fp, fit, ic, prog, _) in enumerate(
        _hof_programs_data[:_TOP_TO_SHOW_PRINT], 1
    ):
        head = textwrap.shorten(prog.to_string(max_len=300), width=90, placeholder="…")
        print(
            f" {rk:>4} | {fit:+7.4f} | {ic:+5.3f} | {prog.size:3d} | {fp[:8]} | {head}"
        )
    print()
