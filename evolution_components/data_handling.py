...
def _rolling_features_individual_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["ret_fwd"] = (
        df["close"].pct_change(periods=1).shift(-1)
    )  # Shift by -1 for ret_fwd
    return df
...
def _load_and_align_data_internal(
    ...
    for csv_file in glob.glob(os.path.join(data_dir_param, "*.csv")):
        try:
            ...
            raw_dfs[Path(csv_file).stem] = (
                df_with_features.dropna()
    ...
    if not raw_dfs:
        sys.exit(
            f"No valid CSV data loaded from {data_dir_param}. Ensure files have 'time' and OHLCV columns."
        )
    ...
    if common_index is None or len(common_index) < required_length_for_data_slice:
        sys.exit(
            f"Not enough common history across all symbols. Need {required_length_for_data_slice} (for {min_common_points_param} eval steps + lag {eval_lag}), got {len(common_index) if common_index is not None else 0})."
        )
    ...
    for sym in symbols_to_keep:
        ...
        if reindexed_df.isnull().values.any():
            print(
                f"Warning: DataFrame for {sym} still contains NaNs after ffill/bfill on common_index. This might indicate issues with source data or alignment logic."
            )
...
def initialize_data(
    ...
    print(
        f"Loading and aligning data: dir='{data_dir}', strategy='{strategy}', min_points='{min_common_points}', eval_lag='{eval_lag}'"
    )
    ...
    print(
        f"Data initialized: {_N_STOCKS} symbols, {_COMMON_TIME_INDEX.size if _COMMON_TIME_INDEX is not None else 0} common time steps."
    )
    if _COMMON_TIME_INDEX is not None:
        print(
            f"Data spans from {_COMMON_TIME_INDEX.min()} to {_COMMON_TIME_INDEX.max()}."
        )
