...
from .alpha_framework_types import (
    OP_REGISTRY,
)  # OpSpec is implicitly used via OP_REGISTRY
...
class Op:
    ...
    def execute(
        self, buf: Dict[str, Union[np.ndarray, float]], n_stocks: int
    ):  # Adjusted buf type hint
        ...
        for i, in_name in enumerate(self.inputs):
            ...
            if arg_val is None:
                raise KeyError(
                    f"Op '{self.opcode}' input variable '{in_name}' not found in buffer. Buffer keys: {list(buf.keys())}"
                )
            ...
            if expected_type == "scalar":
                if isinstance(arg_val, np.ndarray):
                    if arg_val.size == 1:
                        ...
                        processed_args.append(
                            np.mean(arg_val) if arg_val.size > 0 else 0.0
                        )
                ...
                if np.isscalar(arg_val):
                    ...
                    if (
                        arg_val.shape[0] != n_stocks
                        and not spec.is_cross_sectional_aggregator
                    ):
                        if arg_val.size == 1:
                ...
                if not (isinstance(arg_val, np.ndarray) and arg_val.ndim == 2):
                    raise TypeError(
                        f"Op '{self.opcode}' input '{in_name}' expected matrix, got {type(arg_val)} value {arg_val}"
                    )
                processed_args.append(arg_val)
        ...
        if (
            ...
            spec.out_type == "vector"
            and isinstance(result, np.ndarray)
            and result.ndim == 1
            and result.shape[0] != n_stocks
            and not spec.is_cross_sectional_aggregator
        ):
            if result.size == 1:
