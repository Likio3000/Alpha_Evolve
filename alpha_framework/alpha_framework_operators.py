    ...
    *,
    ...
    def _wrapper(fn: Callable):
        ...
        OP_REGISTRY[name] = OpSpec(
            fn, in_types, out, is_cross_sectional_aggregator, is_elementwise
        )
        return fn
...
def _div(a, b):
    ...
    res = np.divide(
        a,
        b_arr,
        out=np.zeros_like(out_shape_ref, dtype=float)
        if hasattr(out_shape_ref, "shape")
        else 0.0,
...
def _sqrt(a):
    return np.sqrt(np.maximum(np.asarray(a), 0))
...
def _power(a, b):
    ...
    * Hard-clips result to ±MAX_ABS so numbers never explode downstream.
    """
    MAX_ABS = (
        1e9  # This MAX_ABS is specific to _power, SAFE_MAX is for general cleaning
    )
    ...
    # ------------------------- array exponent ------------------------- #
        ...
        with np.errstate(over="ignore", invalid="ignore", divide="ignore"):
            ...
            if np.any(mask_signed):
                result[mask_signed] = np.sign(a_arr[mask_signed]) * np.power(
                    np.abs(a_arr[mask_signed]),
                    b_arr[mask_signed],
                )

    # ------------------------- clean-up + clamp ----------------------- #
    result = np.nan_to_num(
        result, nan=0.0, posinf=np.finfo(float).max / 2, neginf=-np.finfo(float).max / 2
    )
    return _clip(result)


@register_op(
    "min_val", in_types=("scalar", "scalar"), out="scalar", is_elementwise=True
)
def _min_val(a, b):
...
@register_op(
    "max_val", in_types=("scalar", "scalar"), out="scalar", is_elementwise=True
)
def _max_val(a, b):
...
# Vector ops
@register_op(
    "cs_mean", in_types=("vector",), out="scalar", is_cross_sectional_aggregator=True
)
def _cs_mean(v):
...
@register_op(
    "cs_std", in_types=("vector",), out="scalar", is_cross_sectional_aggregator=True
)
def _cs_std(v):
    return float(np.std(v, ddof=0)) if v.size > 1 else 0.0
...
def _vec_div_scalar(v, s):
    return v / (s if np.abs(s) > 1e-9 else np.copysign(1e-9, s))
